...
def main():
    ...
    for top in top_level:
        ...
        for tcid, info in agg[tid].items():
            ev = (next((e for e in info.get("evidence", []) if e.get("outcome") == info["outcome"]), None) or (info.get("evidence") or [{}])[0] if info.get("evidence") else {})
            rows.append({
                "TopSuiteId": tid,
                "TopSuiteName": tname,
                "TestCaseId": tcid,
                ...
                "OutcomeSource": ev.get("source"),
                "OutcomeTimestamp": ev.get("ts"),
                "BackfillRunId": ev.get("runId"),
            })
